...
print('You must copy the flare_line_fits.ecsv table into the flarebop/tables directory for updated'
...
for fitdef in fits:
    ...
    xpivot = 10**lxpivot
    eqn = f'$y = {10**b:.2g}\\ (x/{xpivot:.0f})^{{{a:.2f}}}$'
    pct = f'percentile = {fitdef['percentile']}'
    plt.annotate('\n'.join((eqn,pct)), xy=(0.02,0.98), xycoords='axes fraction', ha='left', va='top', fontsize='small')
    plt.tight_layout()

    filename = (f'{fitdef['yparam'].replace(' ', '')}-{fitdef['yband']}.vs.'
                f'{fitdef['xparam'].replace(' ', '')}-{fitdef['xband']}')
    if fitdef['percentile'] != 'n/a':
